...
-(void)label_AutoMaskWithHeightMask:(BOOL)heightMask
{
    ...
    //重设label尺寸
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, heightMask?CGRectGetWidth(self.frame):flt_Auto, heightMask?flt_Auto:CGRectGetHeight(self.frame));
}
 ...
 *
 *  @param heightMask 是否是高度根据文字自适应
 *  @param lineNumber 文字行数
 */
-(void)label_AutoMaskWithHeightMask:(BOOL)heightMask withLineNumber:(NSInteger)lineNumber
{
    //记录自适应后的高度
    float flt_Auto = .0f;
    //当前要适应的尺寸高度
    CGSize size_Content = CGSizeMake(heightMask?CGRectGetWidth(self.frame) - 10 : MAXFLOAT, heightMask?MAXFLOAT:CGRectGetHeight(self.frame));
    //IOS7.0以上计算文字高度
    CGSize size = [self.text boundingRectWithSize:size_Content options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font} context:nil].size;
    //记录计算后的变化的尺寸
    flt_Auto = heightMask?size.height:size.width;
    ...
    float height_One = [@"陈平喆" boundingRectWithSize:size_Content options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font} context:nil].size.height;
    float height = lineNumber == 0?flt_Auto:fminf((height_One * lineNumber), flt_Auto);
    //重设label尺寸
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, heightMask?CGRectGetWidth(self.frame):height, heightMask?height:CGRectGetHeight(self.frame));
}
