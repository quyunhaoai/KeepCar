...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method m3 = class_getInstanceMethod(cls, @selector(viewWillAppear_EverPath));
...
- (void)viewWillAppear_EverPath
{
    [self viewWillAppear_EverPath];
    //   进入
    ...
    NSLog(@"Ever_VC_Path:%s\n",NSStringFromClass(self.class).UTF8String);
    NSLog(@"Super_VC_Path:%s\n",NSStringFromClass([self.class superclass]).UTF8String);
